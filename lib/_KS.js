...
module.exports.processed = item => {
    ...
    year_node = document.evaluate('//*[@id="infoTable"]/table/tbody/tr[1]/td[2]/div/a', document, null, xpath.XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
    if (year_node){
        item.schema['properties']['year'] = year_node.textContent;
    }
    let reviewsData = document.querySelectorAll('.reviewItem.userReview');
    if (reviewsData && reviewsData.length !== 0){
        let reviews = [];
        reviewsData.forEach(reviewData => {
            if (reviewData.querySelector('._reachbanner_').length){
                avatar = '';
                if (reviewData.querySelector('div[itemprop=author] img').length){
                    avatar = reviewData.querySelector('div[itemprop=author] img').dataset.lazySrc
                }
                author = '';
                if (reviewData.querySelector('.profile_name a').length){
                    author = reviewData.querySelector('.profile_name a').innerHTML
                }
                date = '';
                if (reviewData.querySelector('.date').length){
                    date = reviewData.querySelector('.date').innerHTML
                }
                headline = '';
                if (reviewData.querySelector('.sub_title').length){
                    headline = reviewData.querySelector('.sub_title').innerHTML
                }
                reviews.push({
                    "id":  reviewData.dataset.id,
                    "reviewBody": reviewData.querySelector('._reachbanner_').innerHTML,
                    "avatar": avatar,
                    "author": author,
                    "date": date,
                    "headline": headline
        ...
        item.schema['properties']['reviews'] = reviews;
    }
